...
function personCard(c){
  const secretariat = c.telephone ? `<p class="small"><strong>Secrétariat :</strong> <a href="${telHref(c.telephone)}">${c.telephone}</a></p>` : '';
  const actions = [
    c.doctolib ? `<a class="btn primary" href="${c.doctolib}" target="_blank" rel="noopener">Fiche Doctolib</a>` : '',
    c.pubmed ? `<a class="btn secondary" href="${c.pubmed}" target="_blank" rel="noopener">Publications PubMed</a>` : ''
  ].filter(Boolean).join('');
...
function newsCard(n){
  ...
  const image = n.image ? `<img class="news-image" src="${pathPrefix}assets/actualites/${n.image}" alt="${n.titre}" onerror="this.remove()">` : '';
  const link = n.lien ? `<a class="btn secondary" href="${n.lien}" target="_blank" rel="noopener">Lire</a>` : '';
  const texte = n.texte ? `<details><summary>Lire la suite</summary><p>${n.texte}</p></details>` : '';
  return `<article class="card news-card">${image}<time>${d}</time><div class="tag">${n.categorie||'Actualité'}</div><h3>${n.titre}</h3><p>${n.resume}</p>${texte}${link}</article>`;
}
...
async function hydrate(){
  ...
  if(peopleEl || peoplePreviewEl){
    ...
    const sortedPeople = people.sort((a,b)=>(a.ordre_affichage||999)-(b.ordre_affichage||999));
    if(peopleEl) peopleEl.innerHTML = sortedPeople.map(personCard).join('');
    if(peoplePreviewEl) peoplePreviewEl.innerHTML = sortedPeople.slice(0,6).map(personCard).join('');
  }
